...
function ScrollView(props: ScrollViewProps, ref: ForwardedRef<HTMLDivElement>) {
  ...
  const enabledX = scrollbar === undefined || scrollbar === 'x'
  const enabledY = scrollbar === undefined || scrollbar === 'y'
  ...
  const getSliderSize = useCallback(() => {
    ...
    if (result.width === trackWidth) {
      result.width = 0
    }
    if (result.height === trackHeight) {
      result.height = 0
    }
    return result
  }, [getTrackSize])
  ...
  }, [scrollbar, dispatchUpdate, getSliderSize])
  ...
  return (
    ...
    <div
      ...
      className={cx(
        'are-scroll-view',
        { 'are-scroll-view-hidden': display === 'hover' && !hoverShow },
        className,
      )}
      onMouseEnter={(e) => {
    ...
    >
      <div
        ...
        className={cx('are-scroll-view-content')}
      ...
      {enabledX && (
        <div
          ref={trackXRef}
          className={cx(
            'are-scroll-view-track are-scroll-view-track-x',
        ...
        >
          ...
          <div
            ref={sliderXRef}
            className={cx(
              'are-scroll-view-slider are-scroll-view-slider-x',
              {
                'are-scroll-view-slider-dragging':
                  isDragging && prevPageX.current,
              },
              styles?.x?.slider?.className,
            )}
            style={{
              ...styles?.x?.slider?.style,
              width: size.width,
      ...
      {enabledY && (
        <div
          ref={trackYRef}
          className={cx(
            'are-scroll-view-track are-scroll-view-track-y',
        ...
        >
          ...
          <div
            ref={sliderYRef}
            className={cx(
              'are-scroll-view-slider are-scroll-view-slider-y',
              {
                'are-scroll-view-slider-dragging':
